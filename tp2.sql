...
DBMS_OUTPUT.PUT_LINE('Employé : ' || r_employees.FIRST_NAME ||' '|| r_employees.LAST_NAME ||'( ID : '|| r_employees.EMPLOYEE_ID || ')travaille comme ' || r_employees.JOB_TITLE || ' depuis ' || r_employees.HIRE_DATE || ' sous la direction d un manager avec la matricule : ' ||r_employees.MANAGER_ID||').');
